...
from ethernity.qr.capacity import choose_frame_chunk_size, fits_qr_payload
from ethernity.qr.codec import QrConfig, make_qr


class TestQrPayloadCapacity(unittest.TestCase):
    @mock.patch("ethernity.qr.capacity.make_qr", return_value=object())
    def test_fits_qr_payload_true(self, _make_qr: mock.MagicMock) -> None:
        self.assertTrue(fits_qr_payload(b"abc", QrConfig()))

    @mock.patch("ethernity.qr.capacity.make_qr", side_effect=ValueError("too big"))
    def test_fits_qr_payload_false_on_error(self, _make_qr: mock.MagicMock) -> None:
        self.assertFalse(fits_qr_payload(b"abc", QrConfig()))
