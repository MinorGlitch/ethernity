...
def _inputs(
    ...
    *,
    frame_count: int = 3,
    paper_size: str = "A4",
    custom_dimensions_mm: tuple[float, float] | None = None,
) -> RenderInputs:
    return RenderInputs(
        ...
        page_size=(
            PaperSize(
                paper_size,
                paper_size.replace("_", " ").title(),
                *custom_dimensions_mm,
            )
            if custom_dimensions_mm is not None
            else None
        ),
    )
...
class TestDirectPdfForgeMain(unittest.TestCase):
    ...
    def test_render_writes_direct_layout_debug_json_sidecar(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["pages"][0]["qr_count"], 2)
    ...
    def test_build_plan_paginates_qr_payloads(self) -> None:
        with TemporaryDirectory() as tmp:
            inputs = _inputs(Path(tmp) / "main.pdf", frame_count=15)
            surface = FpdfSurface(page_width_mm=A4_WIDTH_MM, page_height_mm=A4_HEIGHT_MM)
            packaged_direct_pdf_assets().register_fonts(surface)
            ...
            continuation_panel = _component(
                continuation_page,
                "forge-main-p2-continuation-panel",
            )
            first_qr_card = _component(continuation_page, "forge-main-p2-qr-card-6")
            final_qr_card = _component(continuation_page, "forge-main-p2-qr-card-14")
    ...
    def test_plan_and_render_are_responsive_to_letter(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            inputs = _inputs(output_path, frame_count=15, paper_size="LETTER")
    ...
    def test_unregistered_portrait_size_uses_dimensions_not_paper_name(self) -> None:
        with TemporaryDirectory() as tmp:
            output_path = Path(tmp) / "main-custom.pdf"
            width_mm = 220.0
            height_mm = 310.0
            inputs = _inputs(
                output_path,
                frame_count=15,
                paper_size="CUSTOM_PORTRAIT",
                custom_dimensions_mm=(width_mm, height_mm),
            )
            ...
            self.assertTrue(all(page.rect.height_mm == height_mm for page in plan.page_plans))
            reader = validate_pdf_has_pages(output_path)
            rendered_width_mm = float(reader.pages[0].mediabox.width) * 25.4 / 72.0
