...
class TestDirectPdfLedger(unittest.TestCase):
            ...
            for passphrase_case, passphrase in _RECOVERY_PASSPHRASE_CASES:
                ...
                for paper_size in _PAPER_DIMENSIONS:
                    with self.subTest(case=passphrase_case, paper_size=paper_size):
                        ...
                        plan = build_ledger_recovery_direct_plan(
        ...
        with TemporaryDirectory() as tmp:
            ...
            for paper_size in _PAPER_DIMENSIONS:
                with self.subTest(paper_size=paper_size, doc_type=DOC_TYPE_MAIN):
                    ...
                    self.assertEqual(len(_page_items(plan.page_plans[-1], "-qr-card-")), 2)
                    self.assertEqual(len(_page_items(plan.page_plans[-1], "-qr-label-")), 2)
                    for page_plan in plan.page_plans:
                        cards = _page_items(page_plan, "-qr-card-")
                ...
                with self.subTest(paper_size=paper_size, doc_type=DOC_TYPE_KIT):
                    ...
                    self.assertEqual(len(plan.page_plans), 3)
                    self.assertEqual(len(_page_items(plan.page_plans[1], "-qr-card-")), 5)
    ...
    def test_tall_recovery_page_measures_four_digit_number_gutter(self) -> None:
        with TemporaryDirectory() as tmp:
            page = PaperSize("LEDGER_TALL", "Ledger tall", 200.0, 10_000.0)
            base_inputs = _recovery_inputs(
                Path(tmp) / "recovery-ledger-tall.pdf",
                data_size=100_000,
            )
            inputs = replace(
                base_inputs,
    ...
    def test_five_digit_number_gutter_preserves_payload_width(self) -> None:
        ...
        plans = ledger_module._fallback_block_plans(
            surface,
            fallback_page,
            prefix="ledger-five-digit",
    ...
    def test_shard_fallback_is_full_width_and_bottom_anchored_on_future_page(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            for doc_type, builder in (
                (DOC_TYPE_SHARD, build_ledger_shard_direct_plan),
                (DOC_TYPE_SIGNING_KEY_SHARD, build_ledger_signing_key_shard_direct_plan),
            ):
                with self.subTest(doc_type=doc_type):
                    base_inputs = _shard_inputs(
                        Path(tmp) / f"{doc_type}-future.pdf",
                        doc_type=doc_type,
                        data_size=2048,
                    )
                    inputs = replace(
