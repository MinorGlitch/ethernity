...
class TestFilterFallbackLines(unittest.TestCase):
    ...
    def test_parse_fallback_frame_accepts_ungrouped_single_line(self) -> None:
        ...
        parsed = parse_fallback_frame([encoded], label="fallback")
        self.assertEqual(parsed.data, b"payload")

    def test_parse_fallback_frame_rejects_invalid_non_empty_lines(self) -> None:
        with self.assertRaisesRegex(ValueError, "outside the z-base-32 alphabet"):
            parse_fallback_frame(["ybndr fghj", "invalid @#$ line"], label="fallback")
