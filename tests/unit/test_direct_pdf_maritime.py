...
class TestDirectPdfMaritime(unittest.TestCase):
    ...
    def test_recovery_metadata_adapts_to_extension_quorum_label(self) -> None:
        recovery_meta = build_recovery_meta(
            passphrase=None,
            quorum_threshold=2,
            quorum_shares=3,
            signing_pub=b"\x31" * 32,
            quorum_label="Extension Shard Quorum",
        )
        with TemporaryDirectory() as tmp:
            for paper_size in _PAPER_DIMENSIONS:
                with self.subTest(paper_size=paper_size):
                    inputs = replace(
                        _recovery_inputs(
                            Path(tmp) / f"recovery-extension-shards-{paper_size}.pdf",
                            paper_size=paper_size,
                        ),
                        recovery_meta=recovery_meta,
                    )
                    ...
                    label = next(
                        ...
                        if item.component_id == "maritime-recovery-p1-meta-label-1"
                    )
                    value = next(
                        item
                        for item in first_page.plans
                        if item.component_id == "maritime-recovery-p1-meta-value-1"
                    )
                    self.assertEqual(
        ...
        with TemporaryDirectory() as tmp:
            for page_size, maximum_pages in page_sizes:
                with self.subTest(page_size=page_size.name):
                    ...
                    content_bottom_mm = maritime_module.resolve_classic_layout(
                        inputs,
                        style=maritime_module._PAGE_STYLE,
