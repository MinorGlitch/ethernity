...
def _recovery_inputs(output_path: Path) -> RenderInputs:
    auth_frame = _frames(FrameType.AUTH, 1, label="maritime-auth")[0]
    main_frame = _frames(FrameType.MAIN_DOCUMENT, 1, label="maritime-recovery-main")[0]
    return RenderInputs(
...
def _shard_inputs(output_path: Path, *, doc_type: str = DOC_TYPE_SHARD) -> RenderInputs:
    frame = _frames(FrameType.KEY_DOCUMENT, 1, label=f"maritime-{doc_type}")[0]
    context = _base_context()
    ...
    def test_render_shard_and_signing_key_shard_write_valid_pdfs(self) -> None:
        with TemporaryDirectory() as tmp:
            for doc_type, expected_title, renderer in (
                (DOC_TYPE_SHARD, "SHARD DOCUMENT", render_maritime_shard_direct_pdf),
                (
                    DOC_TYPE_SIGNING_KEY_SHARD,
