...
class TestPrepareEnvelope(unittest.TestCase):
    ...
    def test_basic_envelope_creation(self) -> None:
        ...
        envelope, payload = _prepare_envelope(input_files, plan, sign_priv, store_signing_key=False)
    ...
    def test_envelope_with_signing_key_stored(self) -> None:
        """Test envelope with signing key stored in manifest."""
        sign_priv, sign_pub = generate_signing_keypair()
        input_files = [
            MockInputFile("data.bin", b"secret data"),
        ]
        plan = DocumentPlan(version=1, sealed=False, sharding=None, signing_seed_sharding=None)

        envelope, payload = _prepare_envelope(input_files, plan, sign_priv, store_signing_key=True)

        manifest, decoded_payload = decode_envelope(envelope)
        self.assertIsNotNone(manifest.signing_seed)
    ...
    def test_multiple_files(self) -> None:
        ...
        self.assertEqual(decoded_payload, b"content1content2content3")
        paths = [f.path for f in manifest.files]
        self.assertEqual(paths, ["file1.txt", "dir/file2.txt", "file3.bin"])
...
class TestRecoverFlow(unittest.TestCase):
    ...
    def test_decrypt_and_extract_integration(self) -> None:
        ...
        from ethernity.cli.core.crypto import _doc_hash_from_ciphertext, _doc_id_from_ciphertext
