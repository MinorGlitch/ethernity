...
class TestStableV1FrozenBaseline(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ensure_playwright_browsers()
        cls._index = json.loads((_GOLDEN_ROOT / "index.json").read_text(encoding="utf-8"))
    ...
    def test_frozen_artifact_hashes_match_snapshots(self) -> None:
        for scenario in self._scenarios():
            with self.subTest(scenario=scenario["id"]):
                ...
                scenario_root = _GOLDEN_ROOT / str(scenario["id"])
    ...
    def _snapshot(self, scenario: dict[str, object]) -> dict[str, object]:
        return json.loads((_GOLDEN_ROOT / str(scenario["path"])).read_text(encoding="utf-8"))
