...
class TaskCanvas(Widget):
        ...
        with Vertical(id="task-canvas-shell"):
            with HorizontalGroup(id="canvas-hero"):
                ...
                with Vertical(id="canvas-readiness"):
                    ...
                    yield ProgressBar(total=1, show_eta=False, id="canvas-progress")
    ...
    def update_task(
        ...
        *,
        ...
        action_bar.display = not is_settings
        action_bar.update_actions(
            ...
            blocked=not validation.ready,
            compact=is_settings,
        )
        self.query_one("#canvas-title", Static).update("" if is_settings else title)
    ...
    def update_settings_values(self, settings: SettingsTaskState) -> None:
        self.query_one(SettingsForm).update_values(settings)
