...
"""Resolve and initialize user-facing config paths."""
...
ONBOARDING_FIELD_RENDER_STYLE = "render_style"
...
def list_render_styles() -> dict[str, Path]:
    ...
    for style_name in SUPPORTED_RENDER_STYLES:
        entry = package_root / style_name
        if not entry.is_dir():
            continue
        if not _is_render_style_dir(entry):
            continue
        styles[style_name] = entry
    return styles


def resolve_render_style_path(style: str) -> Path:
    """Resolve a built-in render style name to a concrete design directory."""

    name = style.strip()
    if not name:
        raise ValueError("render style cannot be empty")
    styles = list_render_styles()
    if name in styles:
        return styles[name]
    lowered = name.lower()
    for candidate, path in styles.items():
        if candidate.lower() == lowered:
            return path
    raise ValueError(f"unknown render style: {style}")


def _is_render_style_dir(path: Path) -> bool:
    """Return whether a directory contains the required built-in style files."""

    return all((path / filename).is_file() for filename in RENDER_STYLE_FILENAMES)
...
def apply_first_run_defaults(
    ...
    *,
    ...
    _ = resolve_render_style_path(design)
...
def _ensure_user_config(paths: ConfigPaths) -> bool:
    """Create user config when missing."""
