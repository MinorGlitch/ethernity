...
def _render_shard(
    ...
    *,
    ...
    shard_path = os.path.join(
        output_dir,
        f"{filename_prefix}-{doc_id.hex()}-{shard.index}-of-{shard.shares}.pdf",
    )
...
def run_backup(
    *,
    ...
    with status("Preparing payload...", quiet=status_quiet):
        envelope, payload = _prepare_envelope(input_files, plan, sign_priv, store_signing_key)
        manifest = envelope_codec_module.decode_envelope(envelope)[1]
