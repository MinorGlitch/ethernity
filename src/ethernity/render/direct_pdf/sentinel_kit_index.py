...
_FIRST_PAGE_ROW_LIMIT = 4
_CONTINUATION_ROW_LIMIT = 14
...
def build_sentinel_kit_index_direct_plan(
    ...
    pages = _paginate_inventory_rows(_inventory_rows(inputs.context))
    page_plans = tuple(
...
def _validate_inputs(inputs: RenderInputs) -> None:
    ...
    if paper_size != "a4":
        raise ValueError("direct Sentinel kit-index renderer currently supports A4 paper only")
...
def _paginate_inventory_rows(rows: Sequence[_InventoryRow]) -> tuple[_InventoryPage, ...]:
    ...
    while cursor < len(row_tuple):
        capacity = _FIRST_PAGE_ROW_LIMIT if page_number == 1 else _CONTINUATION_ROW_LIMIT
...
def _build_page(
    ...
    *,
    ...
    prefix = sentinel_component_prefix(_COMPONENT_BASE, page.page_number)
    ...
    return build_page_plan(page_number=page.page_number, rect=SENTINEL_PAGE_RECT, plans=plans)
...
def _inventory_plans(
    ...
    *,
    ...
    row_height = 17.1 if len(rows) <= _FIRST_PAGE_ROW_LIMIT else 8.6
