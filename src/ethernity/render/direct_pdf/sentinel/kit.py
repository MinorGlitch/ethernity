...
def _qr_grid(page_layout: SentinelPageLayout) -> ResolvedGrid:
    container = page_layout.map_rect(
        PdfRect(
            _QR_CONTENT_X_MM,
            _QR_TOP_MM,
            _QR_CONTENT_WIDTH_MM,
            _QR_ROWS_PER_PAGE * _QR_CARD_HEIGHT_MM + (_QR_ROWS_PER_PAGE - 1) * _QR_CARD_GAP_MM,
        )
    )
    return resolve_grid(
        container,
        GridPolicy(
            ...
            max_rows=_QR_ROWS_PER_PAGE,
            preferred_item_width_mm=(_QR_CONTENT_WIDTH_MM - (_QR_COLUMNS - 1) * _QR_CARD_GAP_MM)
            / _QR_COLUMNS,
    ...
    *,
    ...
    return [
        ...
        TextBox(
            ...
            policy=TextFitPolicy.SHRINK,
            min_size_pt=6.0,
        ).plan(surface, PdfRect(22.0, 272.0, 65.0, 4.5)),
        TextBox(
        ...
        TextBox(
            ...
            min_size_pt=6.0,
        ).plan(surface, PdfRect(123.0, 272.0, 65.0, 4.5)),
    ]
