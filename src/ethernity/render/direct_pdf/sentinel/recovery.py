...
def _paginate_fallback_entries(
    ...
    *,
    ...
    while remaining:
        row_capacity = _fallback_row_capacity(page_number, page_layout)
...
def _fallback_row_capacity(page_number: int, page_layout: SentinelPageLayout) -> int:
    if page_number == 1:
        reference_rect = PdfRect(79.7, 80.5, 115.2, 133.8)
        max_rows = _FIRST_PAGE_MAX_ROWS
    else:
        reference_rect = PdfRect(18.0, 83.0, 174.0, 166.0)
        max_rows = _CONTINUATION_MAX_ROWS
    container = page_layout.map_rect(reference_rect)
    return resolve_grid(
        container,
        GridPolicy(
            max_columns=1,
            max_rows=max_rows,
            preferred_item_width_mm=container.width_mm,
            preferred_item_height_mm=_FIRST_PAGE_ROW_HEIGHT_MM,
            minimum_item_width_mm=container.width_mm,
            minimum_item_height_mm=5.6,
        ),
    ).capacity
