...
def _format_qr_input_error(
    ...
    *,
    ...
    if "scan failed" in lowered and scan_failed_hint:
        return _append_hint(message, scan_failed_hint)
    if ("file not found" in lowered or "unable to read file" in lowered) and file_hint:
        return _append_hint(message, file_hint)
    if default_hint:
        return _append_hint(message, default_hint)
    return message
...
def _read_text_lines(path: str) -> list[str]:
    ...
    normalized_path = expand_user_path(path) or path
    if normalized_path == "-":
...
def _frames_from_fallback_lines(
    ...
    *,
    ...
    if not _contains_fallback_markers(lines):
        return _result(
            [_frame_from_fallback_lines(lines, label="fallback")],
            [],
            notice_sink,
        )
    ...
    if sections["auth"]:
        try:
            ...
            if allow_invalid_auth:
                ...
                raise
    return _result(frames, notices, notice_sink)


def frames_from_fallback(
    path: str,
    *,
    allow_invalid_auth: bool,
    notice_sink: FrameInputNoticeSink | None = None,
...
def _all_lines_match_fallback_text(lines: list[str]) -> bool:
    ...
    if not non_empty:
        return False
    try:
        filtered = _filter_fallback_lines(non_empty)
    except ValueError:
        return False
    return len(filtered) == len(non_empty)
...
def _auth_frames_from_fallback_lines(
    ...
    *,
    ...
    return _result([frame] if frame else [], notices, notice_sink)


def auth_frames_from_fallback(
    path: str,
    *,
    allow_invalid_auth: bool,
    notice_sink: FrameInputNoticeSink | None = None,
...
def recovery_frames_from_scan(
    ...
    *,
    ...
    return _result(recovery_frames, notices, notice_sink)
...
def shard_frames_from_scan(
    ...
    *,
    ...
    return _result(shard_frames, notices, notice_sink)
