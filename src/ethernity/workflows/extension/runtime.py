...
"""Runtime and publish-policy helpers for extension execution."""
...
from ethernity.render.recovery_kit_index import resolve_recovery_kit_index_style
    ...
    *,
    ...
    if passphrase_shard_count <= 0:
        raise ExtensionWorkflowError(
            code=EXTENSION_INVALID_POLICY,
            message=(
                "unlock_policy=self-contained requires extension passphrase shards; "
                "configure --shard-count and --shard-threshold, or use "
                "--unlock-policy reuse-root --shard-count 0 with root shards"
            ),
        )
    assert passphrase_shard_threshold is not None
    passphrase_policy: PassphraseStoragePolicy = ExtensionPassphraseShards(
...
def resolve_extend_runtime(
    ...
    *,
    ...
    config = apply_render_style(
        load_app_config(prepared.args.config_path, paper_size=prepared.args.paper_size),
...
def resolve_root_passphrase_shard_policy(
    ...
    *,
    ...
    root_dir = prepared.args.publish_root
    root_doc_id = prepared.plan.lineage.root_doc_id
    if not root_dir:
        return threshold, count
