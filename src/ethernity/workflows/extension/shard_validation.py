...
"""Shard-carrier validation helpers for extension execution."""
...
from ethernity.workflows.recovery.frame_inputs import shard_frames_from_scan
...
def validate_rendered_shard_carrier(
    *,
    ...
    frames = deduplicate_identical_frames(list(shard_frames_from_scan([str(path)]).frames))
    if len(frames) != 1:
