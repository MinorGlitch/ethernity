...
"""Selected-scope loading and diff helpers for the extension workflow."""
...
def load_selected_scope(args: ExtensionRequest) -> SelectedExtendScope | None:
    """Load validated selected-scope inputs from extend arguments."""

    root_dir = Path(args.publish_root).expanduser().resolve() if args.publish_root else None
    if root_dir is not None:
        _reject_selected_backup_root_artifact_paths(
            raw_files=args.input_paths or (),
            raw_directories=args.input_directories or (),
            root_dir=root_dir,
        )
    scope = load_input_scope(
        raw_files=args.input_paths or (),
        raw_directories=args.input_directories or (),
        base_dir_arg=args.base_directory,
        allow_stdin=True,
    )
