...
"""Safe, adapter-neutral loading of selected input files."""
...
class InputLoadProgress(Protocol):
    ...
    def update(self, task_id: object, **fields: object) -> None: ...
...
@dataclass
class _ScanTracker:
    progress: InputLoadProgress | None
    task_id: object | None
    update_interval: int
    scanned: int = 0
    ...
    *,
    ...
    for raw in input_paths:
        if raw == "-":
            ...
            continue
        path = Path(expand_user_path(raw, preserve_stdin=False) or "")
...
def directory_root_label(path: Path) -> str:
...
__all__ = ["InputFile", "InputLoadProgress", "directory_root_label", "load_input_files"]
