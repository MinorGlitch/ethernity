...
from ethernity.workflows.shared.file_inputs import InputFile, load_input_files
...
def load_input_scope(
    *,
    ...
    """Load validated selected-scope inputs from file and directory selections."""
    ...
    *,
    ...
    for raw in raw_directories:
        directory_prefixes.append(
            _relative_scope_selector(_resolved_user_path(raw), base_dir=base_dir)
        )

    return tuple(sorted(set(exact_paths))), tuple(sorted(set(directory_prefixes)))


def _resolved_user_path(raw: str) -> Path:
    expanded = expand_user_path(raw, preserve_stdin=False)
