...
- Inventory excludes transient/generated directories unless called out explicitly:
  ...
  `src/ethernity/resources/kit/recovery_kit*.bundle.html`, `scripts/__pycache__/`,
  `tests/**/__pycache__/`, `src/.claude/`.
...
- Kit bundle: do not edit or stage generated recovery kit bundles under
  `src/ethernity/resources/kit/`; rebuild them via `kit/build_kit.mjs` before local packaging.
  CI generates these bundles before package and release artifact builds.
...
- Render style capabilities: behavior toggles belong in design style.json files (for example
...
- Render styles: discovery/prompt surfaces must expose only supported built-in style names:
...
- Direct PDF icons: styles that use Material Symbols must resolve packaged local font assets via
  `src/ethernity/render/direct_pdf/assets.py`; do not depend on remote icon CDNs.
- Recovery kit index: backup flow may emit a separate `recovery_kit_index.pdf` when the active
  render style manifest declares `kit_index` support and style capabilities enable
  `recovery_kit_index_document`.
- CLI UI: the human interactive surface is Textual. Do not reintroduce prompt-loop libraries such
  as Questionary or prompt-toolkit.
...
- Entry point and command wiring: `src/ethernity/main.py`, `src/ethernity/app/`, and
  `src/ethernity/run/`.
- Human entry point: `ethernity` launches the Textual terminal app.
- Scriptable command surface: `ethernity run ...` with task commands such as `backup`, `restore`,
  `add-files`, `rebuild`, `replace-recovery-docs`, `print-kit`, and `doctor`.
- Scriptable task command definitions live in `src/ethernity/run/cli.py`.
...
- Direct PDF rendering is isolated in `src/ethernity/render/direct_pdf/` and selected through
  `src/ethernity/render/backend_dispatch.py`.
...
- Built kit outputs (lean + scanner variants) live in `kit/dist/` and are copied into
  `src/ethernity/resources/kit/` by `kit/build_kit.mjs`; both locations are generated and
  ignored locally.
...
- CI (`.github/workflows/ci.yml`) covers Ruff, formatting check, Pyrefly, unit/integration tests,
...
- Backup command help: `uv run ethernity run backup --help`
- Restore command help: `uv run ethernity run restore --help`
- Kit command help: `uv run ethernity run print-kit --help`
