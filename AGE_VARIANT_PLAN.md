...
- Auth frame payload (CBOR map):
  - {"version": auth_version, "hash": doc_hash, "pub": sign_pub, "sig": signature}
...
- Manifest encoding: CBOR maps (canonical CBOR encoding).
- Recovery flow: decrypt -> parse envelope -> restore files using manifest data.
- Manifest layout (CBOR map):
  - {"version": format_version, "created": created_at_epoch, "sealed": sealed, "seed": signing_seed_or_null, "prefixes": prefixes, "files": files}
  - prefixes: ["", "common/dir", ...]
  - files: [{"prefix": prefix_idx, "name": suffix, "size": size_bytes, "hash": sha256_bytes, "mtime": mtime_epoch_int_or_null}, ...]
- Multi-file inputs:
...
- `src/ethernity/crypto/age_runtime.py`: age interface module (pyrage passphrase).
...
- age CLI wrapper (passphrase encrypt/decrypt)
  ...
  - envelope + manifest (CBOR maps, prefix table, sha256 raw bytes, mtime int)
  ...
  - passphrase sharding (Shamir split/join, shard payload format v1, CBOR maps)
