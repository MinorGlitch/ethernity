...
def run_recover_command(args: argparse.Namespace) -> int:
    ...
    cli = _cli_module()
    with _wizard_flow(name="Recovery", total_steps=4, quiet=flow_quiet):
        with _wizard_stage(
            "Recovery input",
            ...
            if args.fallback_file:
                ...
                with _status("Reading fallback text...", quiet=quiet):
                ...
                with _status("Reading frame payloads...", quiet=quiet):
                ...
                with _status("Scanning QR images...", quiet=quiet):
            ...
            if (
                ...
                while True:
                    ...
                    if key_choice == "passphrase":
                        passphrase = _prompt_required_secret(
                            "Enter passphrase",
        ...
        if shard_fallback_files or shard_frame_files:
            with _status("Reading shard inputs...", quiet=quiet):
        ...
        if shard_frames:
            with _status("Reconstructing passphrase from shards...", quiet=quiet):
                passphrase = cli._passphrase_from_shard_frames(
                    shard_frames,
        ...
        if interactive and not quiet:
            with _wizard_stage(
                ...
                review_rows.append(("Auth frames", str(len(auth_frames)) if auth_frames else "none"))
    ...
    if interactive:
        with _wizard_stage(
            ...
            if not quiet:
                ...
                console.print(_panel("Output", _build_review_table(output_rows)))
            if not assume_yes:
                if not _prompt_yes_no(
    ...
    auth_label = _format_auth_status(auth_status, allow_unsigned=allow_unsigned)
    _print_recover_summary(extracted, output_path, auth_status=auth_label, quiet=quiet)
    if not quiet:
        ...
        _print_completion_panel("Recovery complete", actions, quiet=quiet, use_err=True)
    return 0
