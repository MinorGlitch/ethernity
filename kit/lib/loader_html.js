...
export function buildCompressedLoaderHtml({
  ...
  return `<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><script>(async()=>{const p=${JSON.stringify(payload)},a=${JSON.stringify(codecAlphabet)},f=${JSON.stringify(format)},h=${JSON.stringify(fallbackHtml)},w=t=>{document.open();document.write(t);document.close()},d=t=>{let b=0,n=0,v=-1,o=[];for(let i=0,c;i<t.length;i++){c=a.indexOf(t[i]);if(c<0)continue;if(v<0){v=c;continue}v+=c*91;b|=v<<n;n+=(v&8191)>88?13:14;for(;n>7;n-=8,b>>=8)o.push(b&255);v=-1}return v<0?new Uint8Array(o):(o.push((b|v<<n)&255),new Uint8Array(o))};try{if(typeof DecompressionStream!="function")throw 0;w(await new Response(new Blob([d(p)]).stream().pipeThrough(new DecompressionStream(f))).text())}catch{w(h)}})();</script>`;
}
